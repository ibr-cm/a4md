 ...
 */
    struct Position {
        ...
        explicit Position(const libsumo::TraCIPosition &traCiPosition) :
                x(traCiPosition.x * boost::units::si::meter), y(traCiPosition.y * boost::units::si::meter) {};

        libsumo::TraCIPosition toTraCIPosition() const {
            libsumo::TraCIPosition traCiPosition;
            traCiPosition.x = x.value();
            traCiPosition.y = y.value();
            traCiPosition.z = 0;
...
namespace boost {
    namespace geometry {
        namespace traits {
            ...
            template<>
            struct access<artery::Position, 0> {
                static inline double get(const artery::Position &pos) { return pos.x.value(); }

                static inline void
                set(artery::Position &pos, double v) { pos.x = artery::Position::value_type::from_value(v); }
            };

            template<>
            struct access<artery::Position, 1> {
                static inline double get(const artery::Position &pos) { return pos.y.value(); }

                static inline void
                set(artery::Position &pos, double v) { pos.y = artery::Position::value_type::from_value(v); }
            };

            template<>
            struct point_order<std::vector<artery::Position>> {
                static const order_selector value = clockwise;
            };

            template<>
            struct closure<std::vector<artery::Position>> {
