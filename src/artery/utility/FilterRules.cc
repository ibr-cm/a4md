...
namespace artery {
    ...
    FilterRules::FilterRules(omnetpp::cRNG *rng, const Identity &id, std::list<std::string> *applicableServiceNames) :
            mRNG(rng), mIdentity(id), mApplicableServiceNames(applicableServiceNames) {
    }
    ...
    auto FilterRules::createFilterServiceName(const cXMLElement &service_name_cfg) const -> Filter {
        const char *service_name = service_name_cfg.getAttribute("name");
        const char *type_match = service_name_cfg.getAttribute("match");
        bool inverse = type_match && std::strcmp(type_match, "inverse") == 0;
        if (!service_name) {
            throw cRuntimeError("Required name attribute is missing for service filter");
        }
        Filter service_filter = [this, service_name, inverse]() {
            return (std::find(mApplicableServiceNames->begin(), mApplicableServiceNames->end(), service_name) !=
                    mApplicableServiceNames->end()) ^ inverse;
        };
    ...
    bool FilterRules::applyFilterConfig(const omnetpp::cXMLElement &filter_cfg){
        ...
        if (mApplicableServiceNames != nullptr){
            cXMLElementList service_filter_cfg_list = filter_cfg.getChildrenByTagName("service");
            for (cXMLElement *cfg : service_filter_cfg_list) {
                filters.emplace_back(createFilterServiceName(*cfg));
            }
        }

        bool applicable = true;
        if (!filters.empty()) {
            const char *filter_operator = filter_cfg.getAttribute("operator") ? filter_cfg.getAttribute("operator")
                                                                              : "or";
